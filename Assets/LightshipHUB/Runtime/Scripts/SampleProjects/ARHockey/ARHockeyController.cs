...
namespace Niantic.LightshipHub.SampleProjects
{
    ...
    MonoBehaviour
  {
    ...
    //for button press to create game
    ...
    {
        //interactable = false <-- meaning button disable
        startGame.interactable = false;
         _gameStarted.Value = Convert.ToByte(true);
    }

    // This function will be call for both players at line 551
    private void StartGameForAll()
    {
      ...
      _blueScore = _redScore = 0;
        ...
        //find all prefab that has PlayerAvatarBehaviour - can be found in the player prefab
        var players = FindObjectsOfType<PlayerAvatarBehaviour>();
          foreach (var player in players)
          {
                    //assign the colour to the peer and host
            if (player.gameObject != _player) player.SetPlayerColor(_isHost ? "peer" : "host");
          }
            //find all prefab that has BallBehaviour - can be found in the player prefab
      ...
      _playingField = _playingFieldBehaviour.gameObject;
            ...
            //call the function of switchfieldarrows if you are the host
      ...
      _playingFieldBehaviour.UpdateFieldScore(redScore.text, blueScore.text);
        ...
        //prepare the start game screen
      GameOverScreen.SetActive(false);
      GameScreen.SetActive(true);
      
        //count down the game to start
    ...
    IEnumerator StartCountdown() 
    {
      ...
      countdown.gameObject.SetActive(false);
        ...
        //the ball physics starts here by the host
    ...
    {
      if (_manager != null)
        //SendQueuedData is from the High Level Networking API
      ...
      if (_synced && !_gameStart && _isHost)
      {
        ...
        //find a field location like a surface before beginning
      ...
      //if the game haven't start, do not proceed
      if (!_gameStart)
        return;
    ...
    // find a surface to place the field
...
# if UNITY_EDITOR
    ...
    private void OnDidConnect(ConnectedArgs connectedArgs)
    {
      ...
      _gameStarted = new NetworkedField<byte>("gameStarted", authToObserverDescriptor, group);
            ...
            //check if game started or ended
